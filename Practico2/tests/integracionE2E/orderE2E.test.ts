...
describe("Chequea la logica de los metodos de OrderModel con supertest", () => {
    const pizza1 = {size: "S", toppings: ["Mortadela"]}
    const pizza2 = {size: "L", toppings: ["Anana"]}
    it("Se puede crear una order (POST)", async () => { 
        const res = await request(app)
            .post("/order")
            .send({
                address: "Address en algun lugar",
                items: [pizza1, pizza2],
                status: "pending",
                totalPrice: 300
        })       
        ...
        expect(res.body.address).toBe("Address en algun lugar")
    })
    ...
    it("Se puede borrar una order por su id (DELETE)", async () => {
    ...
    it("Se puede modificar el status de una order (PUT)", async () => { 
    ...
    it("No se puede modificar el status de delivered a otro (PUT)", async () => { 
